			...
			$.ajax({
				...
				success : function(data) {
					...
					for(var i = 0; i< length; i++){
						var obj = eval(data)[i];
						total += obj.count*obj.totalPrice;
						computerId += obj.computerid + ",";
						var htmlData = "<tr class='rem1'>\
						<td class='invert-closeb'>\
						<div class='rem'>\
						<div class='close1' onclick='del(" + obj.id +")'></div></div></td>\
						<td class='invert-image'>\
						<a href='single.jsp?computerName=" + obj.computerName + "'>\
						<img src='" + obj.imagePath + "' alt=' ' class='img-responsive' /></a></td>\
						<td class='invert'><div class='quantity'>\
						<div class='quantity-select'><div class='entry value-minus' onclick='sub(" + obj.computerid + "," + obj.count +")'>&nbsp;</div>\
									<div class='entry value'>\
										<span>"+ obj.count +"</span>\
									</div>\
									<div style='display:none'>\
										<input id='rowId' type='text' value='" + obj.computerid +"'>\
									</div>\
									<div class='entry value-plus active'onclick='plus(" + obj.computerid + "," + obj.count +")'>&nbsp;</div>\
								</div></div></td>\
						<td class='invert'>" + obj.computerName +"</td>\
						<td class='invert'>$"+obj.count*obj.totalPrice +"</td>\
					</tr>"
					document.getElementById("cartTable").innerHTML += htmlData;
					}
					/* computerId +="0"; */
					
					$("#orderUser").val(computerId);
					$("#orderAddress").val(computerId);
					$("#orderPhone").val(computerId);
					$("#orderComputer").val(computerId);
					$("#orderPrice").val(total);
		/* 			window.alert("查询之后"+$("#orderComputer").val());
					window.alert($('#orderForm').serialize()); */
				...
				$("#showOrder").click(function(){
					var temp = '<%=session.getAttribute("result")%>';
					//alert(temp);
					if(temp == "null"){
						alert("请先登录");
						$("#myModal4").modal("show");
					}else{
						window.alert("提交"+$('#orderForm').serialize());
						$("#order").modal("show");
				...
				$('#orderButton').click(function() {
					...
					window.alert("提交订单"+$('#orderForm').serialize());
					if ($('#orderForm').data('bootstrapValidator').isValid()) {
						$.ajax({
							type : "post",
							url : "middle/editOrder.jsp",
	...
	<div class="checkout"  >
		<div class="container" >
			...
			<div class="table-responsive checkout-right animated wow slideInUp"
				data-wow-delay=".5s" style="color:white">
			...
			<div class="checkout-left">
				<div class="checkout-left-basket animated wow slideInLeft"
					data-wow-delay=".5s">
					<button id="showOrder" type="button" style="color:white" >提交订单</button>
				</div>
				<div class="clearfix"></div>
			</div>
		</div>
		...
		<div class="modal-dialog">
			<div class="modal-content modal-info">
				<div class="modal-header">
					...
					<h3 class="modal-title" id="myModalLabel">订单页面</h3>
				</div>
								...
								<form id="orderForm" role="form" class="form-horizontal"
									...
									<div class="sign-up form-group" style="display:none">
										...
										<input class="form-control" type="text" placeholder="用户id"
											id="orderUser" name="orderUser">
									</div>
									<div class="sign-up form-group" style="display:none">
										<h4>电脑id :</h4>
										<input class="form-control" type="password" placeholder="电脑id"
											id="orderComputer" name="orderComputer">
									</div>
									<div class="sign-up form-group">
										<h4>收货地址 :</h4>
										<input class="form-control" type="text" placeholder="收货地址"
									...
									<div class="sign-up form-group">
										<h4>总金额 :</h4>
										<input class="form-control" type="text" placeholder="总金额"
											id="orderPrice" name="orderPrice" readonly>
									</div>
									<div class="sign-up">
