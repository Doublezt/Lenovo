...
$.fn.bstree = function(options){
	...
	var res = '\u52a0\u8f7d\u5931\u8d25\uff01';
	var json = qiao.ajax(opt.url + '/tree.do');
	if(json && json.object){
...
bootstrapQ.tree.sub = function(tree, opt){
	...
	if(tree){
		...
		var res = 
			'<li>' + 
				'<a href="javascript:void(0);" class="treeNode" data="id:' + tree.id + ';url:' + tree.url + ';">';
		if(!opt.open && children && children.length > 0){
...
bootstrapQ.tree.init = function(){
	...
	//	}
	//});
	qiao.on('#treeul a', 'click', function(){
		if($(this).find('span').hasClass('glyphicon-minus')){
			if($(this).next().length > 0){
				$('#treeid_' + $(this).qdata().id).collapse('hide');
		...
		if($(this).find('span').hasClass('glyphicon-plus')){
			if($(this).next().length > 0){
				$('#treeid_' + $(this).qdata().id).collapse('show');
