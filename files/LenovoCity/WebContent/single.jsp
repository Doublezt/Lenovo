	...
	$(document).ready(function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				var array = eval(data);
				var obj = eval(data)[0];
				var pictures = obj.totalPicture.split(",");
				//alert(pictures[0]);
				//alert(pictures[4]);
				//alert(obj.id);
				//alert(pictures[0]);
...
<div class='col-md-6 single-right-left animated wow slideInUp animated' data-wow-delay='.5s' style='visibility: visible; animation-delay: 0.5s; animation-name: slideInUp;'>\
			<div class='grid images_3_of_2'>\
				<div class='flexslider'>\
					<ul class='slides'>\
						<li style='display:block;' data-thumb='"+ pictures[0] +"'>\
							<div class='thumb-image'> <img src='" + pictures[0] +"' data-imagezoom='true' class='img-responsive'> </div>\
						</li>\
						<li data-thumb='"+ pictures[1] +"'>\
							<div class='thumb-image'> <img src='"+ pictures[1] +"' data-imagezoom='true' class='img-responsive'> </div>\
						</li>	\
						<li data-thumb='"+ pictures[2] +"'>\
							<div class='thumb-image'> <img src='"+ pictures[2] +"' data-imagezoom='true' class='img-responsive'> </div>\
						</li>\
						<li data-thumb='"+ pictures[3] +"'>\
							<div class=thumb-image'> <img src='"+ pictures[3] +"' data-imagezoom='true' class='img-responsive'> </div>\
						</li>	\
					</ul>\
		...
		<div class='col-md-6 single-right-left simpleCart_shelfItem animated wow slideInRight animated' data-wow-delay='.5s' style='visibility: visible; animation-delay: 0.5s; animation-name: slideInRight;'>\
					<div style='display:none'>\
						<input id='singleId' type='text'  value='" + obj.id + "' >\
					</div>\
					<h3 id='singleName' class='item_name'>"+ obj.name + "</h3>\
					<p>$<span id='singlePrice' class='item_price'>" + obj.price + "</span> </p>\
					<div >\
						<div class='color-quality-right'>\
							<h5>CPU :</h5>\
							<select id='singleCpu' class='frm-field required sect item_cpu' onChange='change()'>\
							</select>\
						</div>\
					</div>\
					<div class='occasional'>\
						<h5>内存 :</h5>\
						<select id = 'singleMemory' class='item_memory frm-field required sect' onChange='change()'>\
						</select>\
					</div>\
					<div class='occasional'>\
						<h5>硬盘 :</h5>\
						<select id = 'singleDisk' class='item_disk frm-field required sect' onChange='change()'>\
						</select>\
					</div>\
					<div class='occasional'>\
						<h5>尺寸 :</h5>\
						<select id = 'singleSize' class='item_size frm-field required sect' onChange='change()'>\
						</select>\
					</div>\
					<div class='occasional'>\
						<h5>数量 :</h5>\
						<input id='singleCount' type='text' value='1' class='item_quantity'>\
					</div>\
					<div class='occasion-cart'>\
					<button class='item_add btn btn-primary btn-sm' onclick='addCart()'>加入购物车</button>\
					</div>\
		</div>\
				<div class='clearfix'> </div>\
				<div class='bootstrap-tab animated wow slideInUp animated' data-wow-delay=''.5s' style='visibility: visible; animation-delay: 0.5s; animation-name: slideInUp;'>\
					<div class='bs-example bs-example-tabs' role='tabpanel' data-example-id='togglable-tabs'>\
						<ul id='myTab' class='nav nav-tabs' role='tablist'>\
							<li role='presentation' class='active'><a href=''#home' id='home-tab' role='tab' data-toggle='tab' aria-controls='home' aria-expanded='true'>产品详情</a></li>\
						</ul>\
						<div id='myTabContent' class='tab-content'>\
							<div role='tabpane' class='tab-pane fade in active bootstrap-tab-text' id='home' aria-labelledby='home-tab'>\
								<h5>商品图片</h5>\
								<img alt='image1' src='"+ pictures[4] +"' style='width:100%'>\
							</div>\
						</div>\
					</div>\
				</div>\
	</div>";
		document.getElementById("showSingle").innerHTML = htmlData;
		open();
		for(var i=0;i<length;i++){
			...
			for(var j=0; j<i;j++){
				...
				if(temp.cpu == array[i].cpu){ flag1 = false;}
				if(temp.hardDisk == array[i].hardDisk){ flag2 = false;}
				if(temp.memory == array[i].memory){ flag3 = false;}
				if(temp.size == array[i].size){ flag4 = false;}
			}
			if(!flag1){
			}else{
			var newCpu = "<option value='" + array[i].cpu + "'>" + array[i].cpu + "</option>";
			document.getElementById("singleCpu").innerHTML += newCpu;
			tempCpu = array[i].cpu;
			}
			if(!flag2){
			}else{
			var newDisk = "<option value='" + array[i].hardDisk + "'>" + array[i].hardDisk + "</option>";
			document.getElementById("singleDisk").innerHTML += newDisk;
			tempDisk = array[i].hardDisk;
			}
			if(!flag3){
			}else{
			var newMemory = "<option value='" + array[i].memory + "'>" + array[i].memory + "</option>";
			document.getElementById("singleMemory").innerHTML += newMemory;
			tempMemory = array[i].memory;
			}
			if(!flag4){
			}else{
			var newSize = "<option value='" + array[i].size + "'>" + array[i].size + "</option>";
			document.getElementById("singleSize").innerHTML += newSize;
			tempSize = array[i].size;
			}
				}
			},
			error : function() {
		...
		if (sresult == "null") {
			alert("请先登录");
			$("#myModal4").modal("show");
		} else if (document.getElementById("singlePrice").innerText == -1) {
			alert("该产品已售空，请重新选择");
		} else {
			$.ajax({
				type : "post",
				url : "middle/addCart.jsp",
				data : {
					userId : result[0],
					computerId : $("#singleId").val(),
					count : $("#singleCount").val(),
					price : document.getElementById("singlePrice").innerText
				},
				success : function(data) {
					alert("添加成功");
				},
	...
	function change() {
		var computerName = document.getElementById("singleName").innerText;
		var cpu = document.getElementById("singleCpu").value;
		var memory = document.getElementById("singleMemory").value;
		var hardDisk = document.getElementById("singleDisk").value;
		var size = document.getElementById("singleSize").value;
		//alert(computerName);
		//alert(cpu);
		$.ajax({
			type : "post",
			url : "middle/changeComputerId.jsp",
			data : {
			...
			success : function(data) {
				//alert(data);
				var info = data.split(",");
				document.getElementById("singlePrice").innerText = info[1];
				$("#singleId").val(info[0]);
			},
			error : function() {
				alert("添加异常");
...
<body onload="load()">
	...
	<!-- 产品页面信息 -->
	<div id="showSingle" class="single"></div>
	<!-- //single -->
